use std::env;
#[cfg(any(feature = "gh-release", feature = "bindgen"))]
...
fn generate_bindings() {
    ...
    let src = format!(
        ...
        bindgen_transform::transform(&raw)
    );
    let out_path = out.join("functions.rs");
    fs::write(&out_path, &src).expect("write functions.rs to OUT_DIR");

    if env::var("Z3_SYS_UPDATE_GENERATED").is_ok() {
        let committed = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap())
            .join("src/generated/functions.rs");
        fs::write(&committed, &src).expect("write committed functions.rs");
        println!("cargo:warning=Updated {}", committed.display());
    }
}
